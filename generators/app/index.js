...
module.exports = class extends Generator {
    async prompting() {
        this.questions = await this.prompt([
            ...
            },
            {
                ...
                name: 'defaultDB',
                message:
                    'database connection will be set to the defaults, is this ok?',
                default: true,
            },
            {
                type: 'input',
    ...
    writing() {
        ...
        if (auth === 'JWT') {
            this.npmInstall(['jsonwebtoken', 'express-jwt', 'bcrypt']);
        }

        if (db === 'noSQL') {
            this.npmInstall(['mongoose']);
            this.npmInstall(['@types/mongoose', 'mongodb-memory-server'], {
                'save-dev': true,
            });
        }
